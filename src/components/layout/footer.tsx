        ...
        <div className="grid md:grid-cols-4 gap-8">
          ...
          <div>
            ...
            <ul className="space-y-1 text-sm">
              ...
              <li><Link href="/map" className="hover:text-primary transition-colors">{t('Directions', 'మార్గదర్శకాలు')}</Link></li>
              <li><Link href="/trust" className="hover:text-primary transition-colors">{t('Family Trust', 'కుటుంబ ట్రస్ట్')}</Link></li>
            </ul>
          </div>
          <div>
            <h3 className="font-headline text-lg font-semibold text-primary mb-2">{t('Contact Info', 'సంప్రదింపు సమాచారం')}</h3>
            <address className="text-sm not-italic text-muted-foreground space-y-2">
              <p className="flex items-start">
                <MapPin className="w-4 h-4 mr-2 mt-0.5 shrink-0 text-primary/80" />
                <span>{t(templeInfo.contact.addressEn, templeInfo.contact.addressTe)}</span>
              </p>
              <p className="flex items-center">
                <Phone className="w-4 h-4 mr-2 shrink-0 text-primary/80" />
                <a href={`tel:${templeInfo.contact.phone}`} className="hover:text-primary transition-colors">{templeInfo.contact.phone}</a>
              </p>
              <p className="flex items-center">
                <Mail className="w-4 h-4 mr-2 shrink-0 text-primary/80" />
                <a href={`mailto:${templeInfo.contact.email}`} className="hover:text-primary transition-colors break-all">{templeInfo.contact.email}</a>
              </p>
            </address>
          </div>
          <div>
